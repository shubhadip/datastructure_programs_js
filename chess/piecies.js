...
function createBoard() {
    startPieces.forEach((e, i) => {
        const element = document.createElement('div');
        element.classList.add('square');
        element.innerHTML = e;
        if(element.firstChild){
            element.firstChild.setAttribute('draggable', true)
        }
        element.setAttribute('square-id', i)
        
        const row = Math.floor( (63 - i) / 8) + 1;
        if(row % 2 === 0) {
            element.classList.add(i % 2 === 0 ? 'biege' : 'brown')
        }else {
            element.classList.add(i % 2 === 0 ? 'brown' : 'biege')
        }

        if(i <= 15) {
            element.firstChild.classList.add('black')
        }
